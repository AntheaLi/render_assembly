...
class PbrtRenderer(object):
    def __init__(self, options=None):
        self.__temporary_folder = Path('.tmp' + str(np.random.randint(100)))
    ...
    def add_tri_mesh(self, tri_mesh, transforms=None, color=(.5, .5, .5), texture_img=None, material=None, obj_with_texture = False):
    ...
    def render(self, verbose=False, nproc=None, light_magnitude = 1.):
        scene_pbrt_name = self.__temporary_folder / 'scene.pbrt'
        # scene_pbrt_name = 'scene.pbrt'
        ...
        with open(scene_pbrt_name, 'w') as f:
            ...
            f.write('LightSource "infinite" "string mapname" "{}" "color scale" [{}, {}, {}]\n'.format(str(self.__lightmap), light_magnitude, light_magnitude, light_magnitude))
        ...
        os.system('convert {}.png {}.jpeg'.format(self.__file_name_only, self.__file_name_only))

        os.remove('{}.exr'.format(self.__file_name_only))
        os.remove('{}.png'.format(self.__file_name_only))
