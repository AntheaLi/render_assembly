...
def export_mp4_jpeg(folder_name, mp4_name, fps, name_prefix=''):
    frame_names = [os.path.join(folder_name, f) for f in os.listdir(folder_name)
        if os.path.isfile(os.path.join(folder_name, f)) and f.startswith(name_prefix) and f.endswith('.jpeg')]
    frame_names = sorted(frame_names)
    ...
    create_folder(tmp_folder, exist_ok=False)
    for i, f in enumerate(frame_names):
        shutil.copyfile(f, tmp_folder / '{:08d}.jpeg'.format(i))

    os.system('ffmpeg -r ' + str(fps) + ' -i ' + str(tmp_folder / '%08d.jpeg') + ' -vcodec libx264 -crf 20 -pix_fmt yuv420p -y ' + str(mp4_name))
