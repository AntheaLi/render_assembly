...
def parse_args():
    ...
    parser.add_argument("--image-dir", default='motions/dataset', type=str, help="A directory of images");
    parser.add_argument("--output-img",  default='forward.png', type=str, help="output image name");
...
def main():
    ...
    # scales = [0.5, 0.5] + [0.25] * 12 # + [0.15] * 8;
    scales = [0.7, 0.7] + [0.25] * 12  + [0.15] * 8;
    ...
    files = [];
    print('dataset length', len(os.listdir(args.image_dir)))
    for fi,f in enumerate(os.listdir(args.image_dir)):
        name, ext = os.path.splitext(f);
        if ext != ".png" and ext != ".jpeg": continue;
    ...
    # alpha_background = Image.new("L", canvas.size, 255)
    # canvas.putalpha(alpha_background)
